...
export default function ViewAllAdminCLubAdmin() {
    ...
    const handleViewClubAdmins = async (club) => {
        ...
        try {
            const response = await axios.get(`http://localhost:7000/api/v1/clubAdmin/allClubsAdmins/${club.clubId}`);
    ...
    const handleDeleteClubAdmin = (clubAdminId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await axios.delete(`http://localhost:7000/api/v1/clubAdmin/delete/${clubAdminId}`);
